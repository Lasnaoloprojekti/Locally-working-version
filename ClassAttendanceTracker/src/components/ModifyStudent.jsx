...
const StudentModification = () => {
  ...
  const handleStudentChange = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://teacher.northeurope.cloudapp.azure.com/api/studenttopics/${studentId}/${selectedCourse}`
      );
      const { courseTopics, attendingTopics } = response.data;

      const nonAttendingTopics = courseTopics.filter(
        (topic) => !attendingTopics.includes(topic)
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.put(
        `https://teacher.northeurope.cloudapp.azure.com/api/updatestudenttopics/${selectedStudent}/${selectedCourse}`,
        {
  ...
  const handleTopicClick = (topic, isParticipating) => {
    if (isParticipating) {
      ...
      setNonParticipatingTopics(
        nonParticipatingTopics.filter((t) => t !== topic)
      );
        ...
        {selectedCourse && selectedStudent && (
          <>
            <div className="flex gap-6">
              <div className="w-1/2">
                <h3 className="font-roboto-slab text-lg mb-2">
                  Topics Attending
                </h3>
                <ul className="pl-5 font-open-sans">
                  {participatingTopics.map((topic) => (
                    <li
                      key={"participating-" + topic}
                      className=" cursor-pointer text-gray-600 hover:text-blue-600"
                      onClick={() => handleTopicClick(topic, true)}>
                      {topic}
                    </li>
                  ))}
                </ul>
              ...
              <div className="w-1/2">
                <h3 className="font-roboto-slab text-lg mb-2">
                  Topics Not Attending
                </h3>
                <ul className="pl-5 font-open-sans">
                  {nonParticipatingTopics.map((topic) => (
                    <li
                      key={"non-participating-" + topic}
                      className=" cursor-pointer text-gray-600 hover:text-blue-600"
