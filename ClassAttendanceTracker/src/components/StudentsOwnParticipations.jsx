...
export const StudentsOwnParticipations = () => {
    ...
    const fetchParticipationData = async () => {
        try {
            const response = await fetch(`http://localhost:3001/api/participation/${studentNumber}`);
        ...
        return (
            <table className="max-w-4xl w-full table-auto mt-7 place-self-center">
                <thead className="bg-blue-900">
                    <tr>
                        <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider font-open-sans">Course</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider font-open-sans">Topic</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider font-open-sans">Your Participation Rate</th>
                    </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-700">
                    {data.map((courseData, courseIndex) => (
                        Object.entries(courseData.participation).map(([topic, rate], topicIndex) => (
                            <tr key={courseIndex + "-" + topicIndex}>
                                {topicIndex === 0 && (
                                    <td className="px-6 py-4 whitespace-nowrap font-roboto-slab" rowSpan={Object.keys(courseData.participation).length}>
                                        {courseData.courseName}
                                    </td>
                                )}
                                <td className="px-6 py-4 whitespace-nowrap font-roboto-slab">{topic}</td>
