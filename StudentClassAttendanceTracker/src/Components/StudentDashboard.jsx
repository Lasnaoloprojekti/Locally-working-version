...
export const StudentDashboard = () => {
    ...
    if (data && !isProcessingScan) {
      ...
      try {
        ...
        const response = await fetch(
          "mdds-server-jj.northeurope.cloudapp.azure.com:3002/qrcoderegistration",
          {
            method: "POST",
            headers: {
  ...
  const fetchParticipationData = async () => {
    try {
      const response = await fetch(
        `mdds-server-jj.northeurope.cloudapp.azure.com:3002/api/participation/${studentNumber}`
      );
