...
const fetchParticipationRates = async (courseId) => {
  try {
    const response = await axios.get(
      `http://mdds-server-jj.northeurope.cloudapp.azure.com:3002/participations/${courseId}`,
      {
...
const submitGdprConsent = async (studentNumber, gdprConsent, userId) => {
  try {
    const response = await axios.post(
      "http://mdds-server-jj.northeurope.cloudapp.azure.com:3002/api/students/updategdpr",
